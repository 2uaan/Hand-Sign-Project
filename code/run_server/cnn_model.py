...
# Giữ nguyên các class Conv3x3_RGB, MaxPool2, ReLU, FullyConnected, Softmax ở trên
# Chỉ thay thế class SignLanguageCNN bên dưới:
...
class SignLanguageCNN:
    def __init__(self, num_classes=4, img_size=96):
        ...
        # 2. TỰ ĐỘNG TÍNH TOÁN KÍCH THƯỚC CHO LỚP FC
        # Không hardcode số 47*47*8 nữa. Hãy để máy tính tự tính.

        # Bước 1: Qua Conv3x3 (Mất 2 pixel viền: N -> N-2)
        # Ví dụ: 96 -> 94
        conv_output_size = self.img_size - 2

        # Bước 2: Qua MaxPool2x2 (Chia đôi kích thước)
        # Ví dụ: 94 -> 47
        pool_output_size = conv_output_size // 2

        # Bước 3: Tính tổng số nút (Nodes) sau khi duỗi phẳng (Flatten)
        # Số node = (Cao x Rộng x Số filters)
    ...
    def forward(self, image):
        # Đảm bảo input đúng shape (H, W, C)
        # Nếu lỡ đưa vào (1, H, W, C) thì bỏ chiều đầu đi
